...
成功会话保存在被 `.gitignore` 忽略的 `cookies/claude_api/`，索引不包含 cookie 值。WebUI 的“Claude API 注册”卡片以及两个编排器都支持 `claude_api`；密码、refresh token 和 client ID 在表单中使用密码框，命令预览统一显示为 `***`。
