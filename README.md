...
- `OCTO_HEADLESS=true` 会全局隐藏所有 Octo profile 窗口；默认 `false`。WebUI 配置页以复选框配置该项；开启后 Octo 客户端仍须保持运行，且无法人工接管 CAPTCHA。
- 该开关仅影响共享 Octo provider，不影响 BitBrowser、AdsPower、IPMart、Clash 或既有 one-time profile 行为；Octo 仍不支持 one-time profile 模式。
...
NINEMALL 的账号源是 `NINEMALL_EMAIL_FILE`（默认 `mail.txt`）；Claude 网页号的消费记录仅写入被忽略的旁路状态文件 `mail_used_claude.txt` 与 `mail_error_claude.txt`，不会写回源文件。账号读取、预留和终态写入由跨进程文件锁串行化；双用途共享预留使用不含凭据的事务日志，任一台账写入失败或进程中断后会按写入前长度回滚，避免只预留了其中一个用途。状态文件通过 fsync 后原子替换发布。

Claude 家族专用流程在启动前选择 NINEMALL：这是默认 provider；显式 `EMAIL_PROVIDER=OUTLOOK` 时则预先选择 OUTLOOK。`register_three_platforms.py --from-pool` 的纯 `claude_api` 会按当前 provider 从用途台账预留账号，同时选择 `claude` 与 `claude_api` 时则对同一邮箱执行原子的双用途预留；为保持旧 Claude.ai 行为，纯 OUTLOOK `claude` 仍使用原有 `tri` 邮箱池。`run_full_flow.py` 仅在 NINEMALL + 纯 Claude 家族时绕过 Stage A 并从用途台账取号；显式 OUTLOOK 仍保留原有 Stage A 邮箱注册行为。选定 NINEMALL 后严格不回退 OUTLOOK、Graph、broker 或 Outlook 浏览器。包含 ChatGPT 或 Grok 的混合流程在启动前直接选择 OUTLOOK，继续使用旧 `tri` 邮箱池和兼容链路；它不会先尝试 NINEMALL，因此不是 NINEMALL 失败后的回退。不要把 NINEMALL 当作跨平台邮箱 provider。
...
### Claude Platform 个人账户注册（`claude_api`）
...
- NINEMALL Claude Platform：`mail_used_claude_api.txt`、`mail_error_claude_api.txt`
- OUTLOOK Claude Platform：`emails_used_claude_api.txt`、`emails_error_claude_api.txt`

成功会话保存在被 `.gitignore` 忽略的 `cookies/claude_api/`。Cookie 文件先以仅所有者可读写的临时文件完成写入和 fsync，再原子发布；索引在跨进程锁内原子更新，且不包含邮箱原文或 cookie 值。WebUI 的“Claude API 注册”卡片以及两个编排器都支持 `claude_api`；密码、refresh token 和 client ID 在表单中使用密码框，命令预览统一显示为 `***`，编排器摘要和转发的子进程日志也会遮蔽完整邮箱地址。包含 `claude_api` 的子流程启动失败、退出非零或缺少成功标记时，编排器返回非零状态；旧的非 `claude_api` 退出语义保持不变。

```powershell
# 单独注册一个 Claude Platform 个人账户
python register_claude_api.py --count 1

# 端到端编排
python run_full_flow.py --platforms claude_api

# 同一邮箱依次注册 Claude 网页号与 Claude Platform
...
启用后，每个候选 SID 都在本地生成并渲染进控制台用户名模板。显式设置 `EMAIL_PROVIDER=OUTLOOK` 时，同一个已验证的 SID 租约贯穿本轮 **Outlook → OAuth token 提取 → Microsoft Graph 邮箱读取 → Claude**，并应用于这些步骤所用的当前指纹浏览器 provider；默认 NINEMALL 流程不会创建 Outlook 或调用 Microsoft Graph。正常首候选成功时恰好发起两次专用 IP 校验请求：获取租约时一次，进入 Claude 前再一次；失败或出口重复才会按 `IPMART_MAX_ATTEMPTS` 更换 SID 重试。出口变化会终止本轮。
...
`IPMART_ENABLED=1` 是 fail-closed：显式 `EMAIL_PROVIDER=OUTLOOK` 时，Outlook → Graph → mailbox → Claude 链路必须取得并验证同一个 IPMart 账号租约，不会回退到 Clash、继承的 HTTP 代理或直连；默认 NINEMALL 的 Claude 家族流程跳过 Outlook Stage A 并严格使用 AppleEmail。ChatGPT 和 Grok 继续保持原有代理行为。
